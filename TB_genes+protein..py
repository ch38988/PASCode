...
############################################################################### 
# prepare data
seed2 = random.randint(1, 2023)
skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=seed2)
cv_accuracy = []
# cv_aucroc = []
iter_num = 0
...
for train_index, test_index in skf.split(train_test_set['donor'], train_test_set[pheno_name]):
    ...
    cv_accuracy.append(np.max(psc.accuracy_test[-1])) # get val accuracy
